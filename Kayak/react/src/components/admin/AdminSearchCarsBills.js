...
class AdminSearchCarsBills extends Component {
  ...
  componentWillMount() {
    this.adminViewCarsBills();
  }

  adminViewCarsBills = () => {

    API.adminViewCarsBills()
      .then((res) => {
  ...
  createRows = (search) => {
    ...
    for (let i = 0; i < search.length; i++) {
      rows.push({
        ...
        from_datetime:search[i].from_datetime,
        to_datetime:search[i].to_datetime,
        booking_date:search[i].booking_date,
        CID:search[i].CID,
        carMake : search[i].carMake,
        carModel: search[i].carModel,
  ...
  render() {
    return (
      <div className="container">
        ...
        <h2 style={{color:'orange'}}><u>CAR BILLS</u></h2>
...
export default withRouter(AdminSearchCarsBills);
